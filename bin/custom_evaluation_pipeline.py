...
parser.add_argument("--shared_results", type=str, nargs="*")
parser.add_argument("--specific_pre_results", type=str, nargs="*")
...
def get_genes(set_id, probesets_file=args.probeset, var_names=adata.var_names):
    ...
    genes = [
        g for g in selection.loc[selection[set_id]].index.to_list() if g in var_names
    ]
    return genes
...
if args.step == "shared":
    evaluator = ProbesetEvaluator(
        adata, metrics=["cluster_similarity"], **evaluator_kwargs
    )
    ...
    evaluator = ProbesetEvaluator(
        adata, metrics=["gene_corr", "marker_corr"], **evaluator_kwargs
    )
    evaluator.compute_or_load_shared_results()

###############################
# 1.2 Compute pre results     #
###############################
...
if args.step == "pre_results_knn":
    for set_id in probesets:
        evaluator = ProbesetEvaluator(
            adata, metrics=["knn_overlap"], **evaluator_kwargs
        )
        genes = get_genes(set_id)
        evaluator.evaluate_probeset(genes, set_id=set_id, pre_only=True)
...
if "probeset_specific" in args.step:
    if args.step == "probeset_specific_fclfs":
        for set_id in probesets:
            evaluator = ProbesetEvaluator(
                adata, metrics=["forest_clfs"], **evaluator_kwargs
            )
            genes = get_genes(set_id)
        ...
        for set_id in probesets:
            genes = get_genes(set_id)
            evaluator.evaluate_probeset_pipeline(
                genes,
                set_id=set_id,
                shared_pre_results_path=args.shared_results,
                step_specific_results=args.specific_pre_results,
            )
...
if args.step == "summary_statistics":
    ...
    evaluator.pipeline_summary_statistics(
        result_files=filtered_results, probeset_ids=probesets
    )
